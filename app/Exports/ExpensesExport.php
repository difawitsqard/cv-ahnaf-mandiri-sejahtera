...
class ExpensesExport implements FromCollection, WithMapping, WithHeadings, WithEvents, WithCustomStartCell
{
    protected $outlet;
    protected $startDate;
    protected $endDate;
    ...
    public function headings(): array
    {
        return [
            ...
            ["Laporan Pengeluaran: " . $this->startDate->format('d F Y') . " s/d " . $this->endDate->format('d F Y')],
            [], // Baris kosong sebelum header tabel
            ['#', 'Pengeluaran', 'Keterangan', 'Oleh', 'Item', '', '', '', '', '', 'Tgl Pengeluaran', 'Total'],
    ...
    public function map($expense): array
    {
        ...
        foreach ($expense->items as $itemIndex => $item) {
            ...
            if ($isFirstRow) {
                ...
                $row[] = ++$numbering; // #
                $row[] = $expense->name; // Nama
                $row[] = strip_tags($expense->description) ?: '-'; // Deskripsi
                $row[] = $expense->user->name; // Oleh
            } else {
            ...
            $row[] = $itemIndex + 1; // No
            $row[] = $item->name; // Name
            $row[] = strip_tags($item->description) ?: '-'; // Keterangan
            ...
            if ($isFirstRow) {
                ...
                $row[] = Carbon::parse($expense->date_out)->format('d F Y H:i'); // Tgl Pengeluaran
                $row[] = $expense->total; // Total
                $isFirstRow = false;
            } else {
